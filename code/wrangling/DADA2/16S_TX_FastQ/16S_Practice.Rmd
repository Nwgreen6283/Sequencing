...
Trim when it drops below 20 or 15 on Quality score (20 is pretty / 15 is ehh)

<!--# Look at a majoruty of them 3-5 / 30 would be a good representation. Or 10% of the number. -->

Lets make cutoff 290
...
Based on these profiles, <!--# we will truncate forwards at 290 and R at 275?? -->
...
```{r}
out <- filterAndTrim(
  fnFs, filtFs, fnRs, filtRs, truncLen=c(290,275), #insert where we trim for F/R
...
The frequency of chimeric sequences varies substantially from dataset to dataset, and depends on on factors including experimental procedures and sample complexity. Here chimeras make up about X% of the merged sequence variants, but when we account for the abundances of those variants we see they account for about 12% of the merged sequence reads.

Maybe more than 25% of chimeras would be bad...
...
#### Assigning Taxonomy from [RDP]{.ul} v18 database

##### Reference database is stored in separate folder in "r_wd" from Project so that I can upload to Github (the ref db is \>100mb)

```{r}
taxa <- assignTaxonomy(seqtab.nochim, "/Users/nathangreenslit/Desktop/UGA/R/r_wd/sequencing_ref_datasets/rdp_train_set_18.fa.gz", multithread=TRUE)
```
