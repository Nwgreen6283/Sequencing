...
sample.names<- substr(fnFs,86,90) #SITE_Day
```
...
<!--# This is where I could merge with all the meta data -->
...
#Day Column:
Day <- substr(subject,4,5)  #Same thing here for date "1" and "2" = 12th 
#day <- as.integer(sapply(strsplit(samples.out, "D"), `[`, 2)) #Does something with the name..


samdf$When <- "HDD"
samdf$When[samdf$Date<13 | samdf$Date== '7_'| samdf$Date== '8_' | samdf$Date== '9_'  ] <- "LDD"

#Date Column
samdf$Date[samdf$Day == '7_'] <- '2022-07-07'
samdf$Date[samdf$Day == '8_'] <- '2022-07-08'
samdf$Date[samdf$Day == '9_'] <- '2022-07-09'
samdf$Date[samdf$Day == '11'] <- '2022-07-11'
samdf$Date[samdf$Day == '12'] <- '2022-07-12'
samdf$Date[samdf$Day == '13'] <- '2022-07-13'
samdf$Date[samdf$Day == '14'] <- '2022-07-14'
samdf$Date[samdf$Day == '15'] <- '2022-07-15'
samdf$Date[samdf$Day == '16'] <- '2022-07-16'
samdf$Date[samdf$Day == '17'] <- '2022-07-17'
samdf$Date[samdf$Day == '18'] <- '2022-07-18'
samdf$Date[samdf$Day == '19'] <- '2022-07-19'

#Type Column
samdf$Type <- "Daily"

#Remove Gut Standard and Day Column
smdf<-
  filter(samdf, !Site == "gu") %>%
  select(!Day)
