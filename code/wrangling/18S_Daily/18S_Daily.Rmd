...
```{r}
path <- "/Users/nathangreenslit/Desktop/UGA/Lipp Lab/Sequencing/data/raw_data/18S_Daily" # CHANGE ME to the directory containing the fastq files after unzipping.
list.files(path)
```
...
<!--# ALSO had to delete RD6_14 R2 file because R1 is missing. I will add back in later. Just going to run the full anayalsis without the sample for now.  -->
...
```{r}
out <- filterAndTrim(
  fnFs, filtFs, fnRs, filtRs, truncLen=c(250,225), #insert where we trim for F/R
...
Top row is length. Bottom row is number of sequences at that length. This makes since as V4 is \~405bp
...
```{r}
taxa <- assignTaxonomy(seqtab.nochim, "/Users/nathangreenslit/Desktop/UGA/R/r_wd/sequencing_ref_datasets/silva_nr99_v138.1_train_set.fa.gz", multithread=TRUE)
...
```{r}
saveRDS(ps_1, file = here("data", "clean_data", "clean_16S_Daily.rds"))

saveRDS(ps, file = here("data", "clean_data", "clean_16s_phyloseq_Daily.rds"))
```
